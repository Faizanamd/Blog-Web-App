...
function Login() {
    const { isAuth, setIsAuth, userId, setUserId, refresh, setRefresh } = useContext(AuthContext);
    const navigate = useNavigate();
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    useEffect(() => {
        setRefresh((prevRefresh) => prevRefresh + 1);
        console.log("refresh in login", refresh);
    }, [])
    async function handleLogin(e) {
        e.preventDefault();

        try {
            const result = await axios.post("https://blog-web-app-backend-ibh1acrfw-faizan-ahmads-projects-1250996e.vercel.app/api/user/login", { email, password })
